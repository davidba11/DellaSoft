...
def generate_invoice_pdf(order_id: int) -> str:
    ...
    with Session(engine) as session:
        ...
        nueva_factura = Invoice(iva=iva, invoice_date=datetime.now(), id_order=order_id)
        ...
        pdf.cell(0, 8, f"Cliente: {customer.first_name} {customer.last_name}", ln=True)
        pdf.cell(0, 8, f"CI: {customer.ci}-{customer.div}", ln=True)
        pdf.ln(4)
        ...
        # Guardar el archivo en public/facturas sin redirigir
        public_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "public", "facturas"))
        os.makedirs(public_dir, exist_ok=True)
        filename = os.path.join(public_dir, f"factura_{nueva_factura.id}.pdf")
        pdf.output(filename)

        return f"/facturas/factura_{nueva_factura.id}.pdf"
