...
class RecipeView(ProductView):
    ...
    create_modal_open: bool = False  # Modal modo crear
    edit_modal_open: bool = False    # Modal modo editar

    selected_product: Optional[Product] = None
    selected_recipe: Optional[Recipe] = None

    form_description: str = ""
    ingredient_rows: List[Dict[str, Optional[int]]] = []  # [{ingredient_id, quantity}]

    _ingredient_options: List[str] = []  # visible en el select
    _ingredient_map: Dict[str, int] = {}  # label -> id_ingredient
    ...
    # Etiqueta seleccionada por fila (para que el select muestre el valor guardado)
    @rx.var
    ...
    def update_quantity(self, index: int, value: str):
        """Permite cantidades decimales ("0.5", "2,75", etc.). Guarda `None` si el texto no es numérico."""
    ...
    async def handle_recipe_click(self, prod_id: int):
        ...
        if not prod:
            return
        if prod_id in self.recipe_map:
            await self.open_edit_modal(prod)
        else:
            self.open_create_modal(prod)

    # ------------------------------------------------------------------
    # Evento "tapón" que envían los Dialog de Radix; evita KeyError
    # ------------------------------------------------------------------
    @rx.event
...
def ingredient_row_component(idx: int):
    return rx.hstack(
        rx.select(
            ...
            value=RecipeView.ingredient_rows_labels[idx],  # ← Var[str]
        ...
        rx.input(
            ...
            value=RecipeView.ingredient_rows_quantities[idx],  # ← Var[str]
...
def create_recipe_form() -> rx.Component:
    return rx.form(
        rx.vstack(
            ...
            rx.button(
                "Añadir ingrediente",
                type="button",  # evita que el form se envíe
            ...
            rx.hstack(
                rx.dialog.close(
                    rx.button("Cancelar", type="button", background_color="#9E9E9E"),
                ),
                rx.dialog.close(
